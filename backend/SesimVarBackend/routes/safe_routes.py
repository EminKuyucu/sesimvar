...
def get_safe_status():
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, latitude, longitude, created_at FROM safe_status WHERE user_id = %s ORDER BY created_at DESC",
            (user_id,)
        )
        ...
        return jsonify({"status": "success", "message": "Veriler getirildi", "data": data}), 200# routes/safe_routes.py

safe_bp = Blueprint('safe', __name__, url_prefix='/user')

# ✅ POST - Güvendeyim Bildirimi
@safe_bp.route('/safe-status', methods=['POST'])
@token_required
def create_safe_status():
    """
